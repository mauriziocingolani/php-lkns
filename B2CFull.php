...
use mauriziocingolani\lkns\classes\BookingIdentifierResponse;
use mauriziocingolani\lkns\classes\CancellationResponse;
use mauriziocingolani\lkns\classes\Company;
use mauriziocingolani\lkns\classes\ConfirmPaymentResponse;
use mauriziocingolani\lkns\classes\Country;
use mauriziocingolani\lkns\classes\ListOfTripsWithDictionaryResponse;
use mauriziocingolani\lkns\classes\Location;
use mauriziocingolani\lkns\classes\PricingResponse;
...
class B2CBasic {
    ...
    public function getLocations($sessionOrParams, string $companyAbbreviations = null, string $locationType = null) {
        ...
        if (is_string($sessionOrParams)) :
            $result = $curl->send($sessionOrParams); elseif (is_array($sessionOrParams)) :
    ...
    public function doTrips($sessionOrParams) {
        ...
        $curl = new Curl($url);
        if (is_string($sessionOrParams)) :
            $result = $curl->send($sessionOrParams, $body); elseif (is_array($sessionOrParams)) :
